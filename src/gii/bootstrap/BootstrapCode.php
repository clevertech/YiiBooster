		...
		if ($column->type === 'boolean') {
			...
			if ($column->type !== 'string' || $column->size === null) {
				if($column->dbType == 'date') {
					return "\$form->datepickerRow(\$model,'{$column->name}',array('class'=>'span5','prepend'=>'<i class=\"icon-calendar\"></i>','append'=>'Click on Month/Year at top to select a different year or type in (mm/dd/yyyy).'))";
				} else {
				...
				if (strpos($column->dbType,'enum(') !== false) {
					...
					return "\$form->{$inputField}(\$model,'{$column->name}',array('class'=>'input-large ','maxlength'=>$column->size))";
				}
			}
		}
	}
